...
export function Aranceles(ArancelesProps: ArancelesProps) {
    const { valorMatricula, valorFinal, porcentaje, data, cargando, error } = useArancel({
        codcar: ArancelesProps.codcar.toString(),
        ...
        <section className="border-(--azul-ucasal) px-6 py-4 rounded-lg border bg-white">
                ...
                <div>
                    ...
                    <h2 className="text-3xl font-bold">
                        {porcentaje && porcentaje > 0?
                            <>
                                <span className="text-gray-500 line-through text-xl">${valorMatricula?.toLocaleString('es-AR')}</span>
            ...
            {cargando &&
                <div className="text-center flex justify-center">
                    <p className="mt-2">Cargando aranceles...</p>
            ...
            {error && <p>Llena el formulario y consulta los aranceles con un asesor</p>}
        </section>
    )
}